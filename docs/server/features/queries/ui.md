...
- `schema_name`: Event type
- `stream_revision`: Event number within the stream
...
The query engine exposes one table for the default index `kdb.records` and one table per user-defined table `usr.<index-name>`.
...
2. In the query editor, enter your SQL query using the tables and fields described above.
