...
public interface ISnapshotProjectionsStore {
    ValueTask<(TSnapshot Snapshot, RecordPosition Position, DateTimeOffset Timestamp)> LoadSnapshot<TSnapshot>(StreamId snapshotStreamId) where TSnapshot : class, new();
    ValueTask SaveSnapshot<TSnapshot>(StreamId snapshotStreamId, StreamRevision expectedRevision, DateTimeOffset Timestamp, TSnapshot snapshot) where TSnapshot : class, new();
}
...
public class SystemSnapshotProjectionsStore(
    ...
    public async ValueTask SaveSnapshot<TSnapshot>(StreamId snapshotStreamId, StreamRevision expectedRevision, DateTimeOffset timestamp, TSnapshot snapshot) where TSnapshot : class, new() {
