...
namespace KurrentDB.Core.Tests.Services.ElectionsService.LeaderNode;
...
public class ElectionsServiceUnitTests {
	...
	public void Setup() {
		...
		for (int i = 0; i < 3; i++) {
			var inputBus = new SynchronousScheduler($"ELECTIONS-INPUT-BUS-NODE-{i}");
			var outputBus = new SynchronousScheduler($"ELECTIONS-OUTPUT-BUS-NODE-{i}");
