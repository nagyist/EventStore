...
internal record struct UserIndexInFlightRecord<TField>(
	long LogPosition,
	long CommitPosition,
	long EventNumber,
	long Created,
...
internal class UserIndexInFlightRecords<TField>(SecondaryIndexingPluginOptions options) {
	private readonly UserIndexInFlightRecord<TField>[] _records = new UserIndexInFlightRecord<TField>[options.CommitBatchSize];
	...
	private bool TryRead(uint currentVer, int index, out UserIndexInFlightRecord<TField> record) {
	...
	public (List<IndexQueryRecord>, bool) GetInFlightRecordsForwards(
		...
		int maxCount,
		bool excludeFirst,
		Func<UserIndexInFlightRecord<TField>, bool>? query = null) {
	...
	private static bool True(UserIndexInFlightRecord<TField> record) => true;

	public IEnumerable<UserIndexInFlightRecord<TField>> GetInFlightRecords() {
		var currentVer = _version;
