...
using KurrentDB.SourceGenerators.Messaging;
	...
	public async Task Impartial() =>
		await RunTestAsync(
			...
			new DiagnosticResult("CS0101", DiagnosticSeverity.Error)
				.WithSpan(
					$"KurrentDB.SourceGenerators{Path.DirectorySeparatorChar}" +
					$"KurrentDB.SourceGenerators.Messaging.{nameof(MessageSourceGenerator)}{Path.DirectorySeparatorChar}" +
					$"Test1-{GenHash(1)}.g.cs", 7, 15, 7, 16)
				.WithArguments("A", "KurrentDB.SourceGenerators.Tests.Messaging.Impartial"));
		...
		await RunTestAsync(
			...
			new DiagnosticResult("CS0101", DiagnosticSeverity.Error)
				.WithSpan(
					...
					$"KurrentDB.SourceGenerators.Messaging.{nameof(MessageSourceGenerator)}{Path.DirectorySeparatorChar}" +
					$"Test1-{GenHash(1)}.g.cs", 7, 15, 7, 16)
				.WithArguments("N", "KurrentDB.SourceGenerators.Tests.Messaging.ImpartialNested"));
